    ...
    <div class="bg-white rounded-lg shadow-sm p-6 sm:p-8">
        ...
        <form action="{{ route('groups.expenses.update', ['group' => $group, 'expense' => $expense]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                >
                    <option value="Other" {{ old('category', $expense->category ?? 'Other') === 'Other' ? 'selected' : '' }}>📝 Other (Miscellaneous)</option>
                    <option value="Accommodation" {{ old('category', $expense->category) === 'Accommodation' ? 'selected' : '' }}>🏨 Accommodation (Hotels, Airbnb, hostels)</option>
                    <option value="Food & Dining" {{ old('category', $expense->category) === 'Food & Dining' ? 'selected' : '' }}>🍽️ Food & Dining (Restaurants, cafés, takeaways)</option>
                    <option value="Groceries" {{ old('category', $expense->category) === 'Groceries' ? 'selected' : '' }}>🛒 Groceries (Supermarket, cooking supplies)</option>
                    <option value="Transport" {{ old('category', $expense->category) === 'Transport' ? 'selected' : '' }}>✈️ Transport (Flights, trains, buses, taxis, Uber)</option>
                    <option value="Activities" {{ old('category', $expense->category) === 'Activities' ? 'selected' : '' }}>🎫 Activities (Sightseeing, tickets, tours, events)</option>
                    <option value="Shopping" {{ old('category', $expense->category) === 'Shopping' ? 'selected' : '' }}>🛍️ Shopping (Clothes, souvenirs, personal items)</option>
                    <option value="Utilities & Services" {{ old('category', $expense->category) === 'Utilities & Services' ? 'selected' : '' }}>⚙️ Utilities & Services (Wi-Fi, laundry, tips)</option>
                    <option value="Fees & Charges" {{ old('category', $expense->category) === 'Fees & Charges' ? 'selected' : '' }}>💳 Fees & Charges (Booking fees, taxes)</option>
                </select>
                @error('category')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
